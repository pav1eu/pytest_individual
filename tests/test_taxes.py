...
@pytest.mark.parametrize('tax_rate, expected', [(10, [110, 220 ,330]),
                                                (15, [115, 230, 345]),
                                                (20,[120, 240, 360]),])
def test_calculte_taxes(prices, tax_rate, expected):
    assert calculate_taxes(prices, tax_rate) == expected


def test_calculte_taxes_invalid_tax_rate(prices):
    with pytest.raises(ValueError):
        calculate_taxes(prices, tax_rate=-1)


def test_calculte_taxes_invalid_prices():
    with pytest.raises(ValueError):
...
def test_calculate_tex_invalid_price():
    with pytest.raises(ValueError):
        calculate_tax(-1, 10)


def test_calculate_tex_invalid_tax_rate_bellow_zero():
    with pytest.raises(ValueError):
        calculate_tax(100, -1)

def test_calculate_tex_invalid_tax_rate_after_100():
    with pytest.raises(ValueError):
